...
121 张带人工评分的测试图的逐片检测/打分记录：批次概要（可判数、检测异常
说明、分布统计与人工分一致性）+ 逐片明细表（人工分/系统参考分/位置评分/
ρu/加权覆盖度/主导支路/X0.95/CCP/备注）。数值取自
...
def _row(r: dict) -> str:
    """明细表一行：编号/人工/系统参考/评分/ρu/A_w/支路/X0.95/CCP/备注。"""
    ...
    return (f"{r['no']:03d}　　{r['man']:>3.0f}　　{r['sys']:>5.1f}　　{r['U']:>5.1f}　　"
            ...
            f"{r['ccp']:.3f}{'　← ' + note if note else ''}")


_HEADER = ("编号　　人工　　系统参考　　评分　　 ρu　　　A_w　　支路　　X0.95　　　CCP")
...
def report_pages() -> list[list[tuple[str, str]]]:
    ...
    pages = [[
        ...
        ("h1", "应力斑位置评分 · 测试集检测报告"),
        ("space", "0.008"),
        ("cnote", f"{DATE}　|　算法 {ALGO_VER}　|　数值与技术说明第六部分同源"),
        ("space", "0.020"),
        ("h2", "批次概要"),
        ("body",
        ...
        ("space", "0.008"),
        ("body",
         f"打分一致性：位置评分与人工评分秩相关（Spearman）= {V['spearman']}；评分范围\n"
         f"[{V['u_min']}, {V['u_max']}]，低于 60 分 {V['n_below_60']} 张；八个人工分档的评分均值随人工分\n"
         "严格单调（下表）。"),
        ("space", "0.008"),
        ("tbl",
