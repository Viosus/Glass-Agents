...
def formula_doc_pages() -> list[list[tuple[str, str]]]:
    ...
    return [
        ...
        [
            ...
            ("plain",
             "【怎么理解】暗场拍照时没玻璃的地方是黑的、玻璃边缘发亮。这条公式定\n"
             "\"多亮才算玻璃\"：两道保险取更严的那道——一道按背景毛糙程度放大，\n"
             "一道按整图亮度差兜底（背景特别干净时防止门槛塌进噪声里）。"),
            ("body",
        ...
        [
            ...
            ("body",
             ...
             "换成\"和本片自己比\"的口径，再走第 6 节的罚分（专属刻度锚）。\n"
             "均匀度的判定口径是独立固定的——调\"判斑灵敏度\"等参数不会牵动它\n"
        ...
        [
            ("space", "0.04"),
            ("h2", "9. 子分换算与加权总分"),
            ("body", "六项指标各自按\"好/差参考值\"换算成 0–100 子分（换算后全部越高越好）："),
            ("formula",
