...
② 位置指标深浅解耦：同图案 ×k 加深 → position_score/ρu 不变（±ε），深浅指标变差；
   全白/常量图 → position_score=100（ρu=0）、方差/梯度≈0、总分≈满分；
...
def test_constant_sheet_position_score_100_and_near_perfect_total():
    # 全白/常量图（用户锚定语义）：position_score=100、ρu=0；方差/梯度≈0；总分≈满分
...
def test_weight_adjustment_changes_total():
    # 工厂调权生效：把 position_score 权重拉满、其余归零 → 总分=position_score 子分
    img = make_glass_image(blobs=[(0.5, 0.5, 0.10, 25.0)], seed=69)
    cfg = indicators_config()
    cfg["indicators"]["weights"] = {
        k: (1.0 if k == "position_score" else 0.0) for k in INDICATOR_KEYS
    }
    ind = compute_sheet_indicators(img, full_interior(img), cfg)
    assert ind.total_score == pytest.approx(ind.sub_scores["position_score"])
...
def test_ccp_sub_score_overflow_and_floor():
    ...
    base = {"x095": 100.0, "gray_variance": 1.0, "gradient_mean": 3.0,
            "gradient_variance": 10.0, "position_score": 100.0}
...
def test_score_from_raw_new_weights_matches_fresh_compute():
    # 换一套权重：快路径重算 == 用新权重全量重跑（position_score 等原始值不受权重影响）
    from fringe_scoring.indicators import score_from_raw
    ...
    new_weights = {k: (2.0 if k == "position_score" else 0.5) for k in INDICATOR_KEYS}
...
def test_position_score_default_ref_equals_passthrough():
    # position_score 参考默认 {best:100, worst:0} == 直通（旧配置缺该键也直通）
    from fringe_scoring.indicators import score_from_raw
    ...
    refs_with["position_score"] = {"best": 100.0, "worst": 0.0}
    ...
    assert sub_b["position_score"] == pytest.approx(sub_a["position_score"])
    assert total_b == pytest.approx(total_a)


def test_position_score_custom_ref_maps_direction():
    # 自定义位置评分参考 {best:100, worst:50}：原始 75 → 子分 50（越大越好方向自动成立）
    from fringe_scoring.indicators import score_from_raw

    raw = {"x095": 20.0, "gray_variance": 1.0, "gradient_mean": 3.0,
           "gradient_variance": 10.0, "ccp": 0.4, "position_score": 75.0}
    cfg = indicators_config()
    refs = dict(cfg["indicators"]["refs"])
    refs["position_score"] = {"best": 100.0, "worst": 50.0}
    sub, _ = score_from_raw(raw, refs, cfg["indicators"]["weights"])
    assert sub["position_score"] == pytest.approx(50.0)
...
def test_select_refs_picks_band_and_falls_back():
    ...
    # 档内未给的指标继承默认参考（如 ccp/position_score）
...
# ---------------- ⑥ 位置指标与斑纹判定解耦（2026-07-08 需求） ----------------
def test_position_indicator_decoupled_from_fringe_judgment_params():
    # 斑纹判定参数（min_dev_gray/top_fraction/min_z/饱和值）怎么调，
    # 位置评分与五项原始指标一律逐位不变——判定只影响掩膜/惩罚，不进六指标原始值；
    # 位置指标掩膜口径已固定（_POSITION_SEG_DEFAULTS），不再继承主 segment
    ...
    for mutate in variants:
        ...
        ind = compute_sheet_indicators(img, full_interior(img), cfg)
        assert ind.position_score == ind0.position_score, f"位置评分被 {mutate} 牵连"
        assert ind.center_concentration == ind0.center_concentration, f"ρu 被 {mutate} 牵连"
        for k, v in ind0.raw_values.items():
            assert ind.raw_values[k] == v, f"{k} 被 {mutate} 牵连"


def test_position_segment_override_is_effective():
    # 解耦不是写死：工厂可经 indicators.position_segment 显式改位置指标口径；
    # 旧键 uniformity_segment（deprecated）仍须等效识别
    ...
    cfg = indicators_config()
    cfg["indicators"]["position_segment"] = {"top_fraction": 0.45, "min_z": 0.5}
    ind_wide = compute_sheet_indicators(img, full_interior(img), cfg)
    assert ind_wide.position_score != ind_default.position_score  # 显式覆写必须生效
    cfg_legacy = indicators_config()
    cfg_legacy["indicators"]["uniformity_segment"] = {"top_fraction": 0.45, "min_z": 0.5}
    ind_legacy = compute_sheet_indicators(img, full_interior(img), cfg_legacy)
    assert ind_legacy.position_score == ind_wide.position_score  # 旧键回退等效


def test_rho0_legacy_key_fallback_and_missing_raises():
    # ρ0 搬迁到 indicators.scoring.position_ratio_at_zero：旧键 uniformity_ratio_at_zero
    # 仍须等效识别（deprecated）；两键皆缺 → 显式 ValueError，不静默给分
    img = make_glass_image(blobs=[(0.5, 0.5, 0.10, 25.0)], seed=87)
    ind_new = compute_sheet_indicators(img, full_interior(img), indicators_config())
    cfg_legacy = indicators_config()
    del cfg_legacy["indicators"]["scoring"]
    cfg_legacy["indicators"]["uniformity_ratio_at_zero"] = 0.33
    ind_legacy = compute_sheet_indicators(img, full_interior(img), cfg_legacy)
    assert ind_legacy.position_score == ind_new.position_score  # 旧键等效（位级）
    cfg_none = indicators_config()
    del cfg_none["indicators"]["scoring"]
    with pytest.raises(ValueError, match="position_ratio_at_zero"):
        compute_sheet_indicators(img, full_interior(img), cfg_none)
